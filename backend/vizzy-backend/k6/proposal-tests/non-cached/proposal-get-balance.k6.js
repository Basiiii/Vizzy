...
// Custom metrics
const proposalGetBalanceTrend = new Trend(
  'proposal_get_balance_duration_no_cache',
  true,
);
...
export const options = {
  ...
  thresholds: {
    ...
    proposal_get_balance_duration_no_cache: ['p(95)<400', 'p(99)<800'],
    errors: ['rate<0.01'],
  },
  setupTimeout: '30s',
};
...
/**
 * Tests the GET /proposals/balance endpoint with cache skipping
 *
 * Test scenarios:
 * 1. Get proposal balance for existing user
 *
 * Metrics tracked:
 * - Response times
 * - Error rates
...
export default function (data) {
  ...
  // Test getting proposal balance
  const res = http.get(`${BASE_URL}/proposals/balance`, {
    headers: headers,
    tags: { name: 'ProposalGetBalanceNoCache' },
    expectedStatuses: [200],
  });

  check(res, {
    'Get Proposal Balance (No Cache): status is 200': (r) => r.status === 200,
    'Get Proposal Balance (No Cache): has balance field': (r) =>
      typeof r.json('balance') === 'number',
  });

  proposalGetBalanceTrend.add(res.timings.duration);

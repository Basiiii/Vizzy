...
// Custom metrics
const proposalGetByIdTrend = new Trend(
  'proposal_get_by_id_duration_no_cache',
  true,
);
const proposalGetByIdNotFoundTrend = new Trend(
  'proposal_get_by_id_not_found_duration_no_cache',
  true,
);
...
export const options = {
  ...
  thresholds: {
    ...
    proposal_get_by_id_duration_no_cache: ['p(95)<500', 'p(99)<800'],
    proposal_get_by_id_not_found_duration_no_cache: ['p(95)<450', 'p(99)<900'],
    errors: ['rate<0.01'],
  },
  setupTimeout: '30s',
};
...
/**
 * Tests the GET /proposals/:proposalId endpoint with cache skipping
 *
 * Test scenarios:
...
export default function (data) {
  ...
  const resExist = http.get(`${BASE_URL}/proposals/${data.testProposalId}`, {
    headers: headers,
    tags: { name: 'ProposalGetByIdNoCache' },
    expectedStatuses: [200],
  });

  const existingCheck = check(resExist, {
    'Get Existing Proposal (No Cache): status is 200': (r) => r.status === 200,
    'Get Existing Proposal (No Cache): has id field': (r) =>
      r.json('id') !== undefined,
    'Get Existing Proposal (No Cache): has required fields': (r) =>
  ...
  const resNotExist = http.get(
    ...
    {
      headers: headers,
      tags: { name: 'ProposalGetByIdNotFoundNoCache' },
      expectedStatuses: [404, 403],
    },
  );

  check(resNotExist, {
    'Get Non-Existing Proposal (No Cache): status is 404 or 403': (r) =>

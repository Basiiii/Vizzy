...
// Custom metrics
const userLocationTrend = new Trend('user_location_duration_no_cache', true);
...
export const options = {
  ...
  thresholds: {
    ...
    user_location_duration_no_cache: ['p(95)<400', 'p(99)<800'],
    errors: ['rate<0.01'],
  },
  setupTimeout: '30s',
};
...
/**
 * Tests the GET /users/location endpoint with cache skipping
 *
 * Test scenarios:
 * 1. Get authenticated user's location (should return 200)
 *
 * Metrics tracked:
 * - Response times
 * - Error rates
...
export default function (data) {
  ...
  const res = http.get(`${BASE_URL}/users/location`, params);

  check(res, {
    'Get User Location (No Cache): status is 200': (r) => r.status === 200,
    'Get User Location (No Cache): has required fields': (r) =>
      r.json('id') !== undefined &&
      r.json('full_address') !== undefined &&
      r.json('lat') !== undefined &&
      r.json('lon') !== undefined &&
      r.json('created_at') !== undefined,
  });

  userLocationTrend.add(res.timings.duration);

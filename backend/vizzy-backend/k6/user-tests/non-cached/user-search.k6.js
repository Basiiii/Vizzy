...
// Custom metrics
const searchDurationTrend = new Trend('user_search_duration_no_cache', true);
const searchResultsTrend = new Trend('user_search_results_no_cache', true);
...
export const options = {
  ...
  thresholds: {
    ...
    user_search_duration_no_cache: ['p(95)<400', 'p(99)<800'],
    errors: ['rate<0.01'],
  },
};

/**
 * Tests the GET /users/search endpoint with cache skipping
 *
 * Test scenarios:
...
export default function () {
  ...
  check(searchRes, {
    'Search (No Cache): status is 200': (r) => r.status === 200,
    'Search (No Cache): has results array': (r) =>
      Array.isArray(r.json('results')),
    'Search (No Cache): has pagination info': (r) =>
  ...
  check(paginationRes, {
    'Pagination (No Cache): status is 200': (r) => r.status === 200,
    'Pagination (No Cache): correct page size': (r) =>
      r.json('results').length <= 5,
  });
  ...
  check(sortRes, {
    'Sort (No Cache): status is 200': (r) => r.status === 200,
    'Sort (No Cache): has sorted results': (r) => {
